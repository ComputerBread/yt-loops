...
panel.innerHTML = '<button onclick="setLoopTimestamp(0)">set start</button><input id="in-start-timestamp" name="in-start-timestamp" /><button onclick="setLoopTimestamp(1)">set end</button><input id="in-end-timestamp" name="in-end-timestamp" /><div></div><button onclick="addLoop()">add loop</button></div><div class="bold style-scope yt-formatted-string">Loops</div><div id="yt-loop-27100145-loop-list"></div><button onclick="startLoop()">start loop</button>';

document.getElementById("columns").before(panel);
